...
if uploaded_file is not None:
    ...
    if COIL_ID_COL in df.columns:
        ...
        # Fallback: treat each row as unique coil
...
* 📊 **Quantified Impact:** Defective coils have a {top_driver} that is on average **{abs(gap_val):.1f} {direction}** than good coils.
    ...
    with tab5:
        ...
        if col_length not in df_filtered.columns or col_scrap not in df_filtered.columns:
            ...
            # -------------------------------------------------------
            # COIL-ID DEDUPLICATION
            #
            # Problem: one coil can appear multiple times in the data
            # (run through the line again after partial processing).
            # e.g.  Pass 1: length=650, scrap=25
            #        Pass 2: length=450, scrap=35
            #
            # Wrong: Total_Length = 650+450 = 1100  (double-counts)
            # Right: Original length (1st pass) = 650
            #        Total scrap = 25+35 = 60
            #        Scrap Rate = 60/650 = 9.23%
            #
            # Implementation:
            #   - Sort by date so the first occurrence = original length
            #   - Per coil: take length from 1st row, sum all scrap rows
            #   - Carry the Time_Group / Thickness / Material from 1st row
            # -------------------------------------------------------
            ...
            # First-pass attributes (original length + metadata)
            coil_first = (
                df_scrap_sorted
            ...
            # Sum of all scrap across every pass
            coil_scrap_sum = (
                df_scrap_sorted
            ...
            # Count how many passes each coil had
            coil_pass_count = (
                df_scrap_sorted
            ...
            # --- Info banner: how many coils had multiple passes ---
            ...
            # ================================================
            # SECTION 3: COMPARISON — RAW vs COIL-AWARE
            # ================================================
            st.markdown("---")
            ...
            # Raw (old) method: just sum all rows as-is
    ...
    if st.sidebar.button("🖨️ Generate PDF Report"):
        try:
            ...
            if tab5_files:
                add_images_to_pdf(
                    pdf, tab5_files,
                    "6. SCRAP ANALYSIS - COIL-ID AWARE (TAB 5)"
                )
